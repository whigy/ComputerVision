function [ segm, prior ] = graphcut_segm(I, area, K, alpha, sigma)
% area = [ minx, miny, maxx, maxy ]

[h,w,c] = size(I);
dw = area(3) - area(1) + 1;
dh = area(4) - area(2) + 1;
mask = uint8([zeros(area(2)-1,w); zeros(dh,area(1)-1), ones(dh,dw), ...
	     zeros(dh,w-area(3)); zeros(h-area(4),w)]); % Initial mask is a rectangle
Ivec = single(reshape(I, size(I,1)*size(I,2), 3));

grey = single(rgb2gray(I));
h = fspecial('gaussian', [7,7], 0.5);
grey = imfilter(grey, h); % apply a 7*7 Gaussian Filter
h = fspecial('sobel'); 
dx = imfilter(grey, h/4);
dy = imfilter(grey, h/4');
grad = sqrt(dx.^2 + dy.^2); %apply a Sobel operater and take the magnitude
edge = (alpha*sigma)*ones(size(grey)) ./ (grad + sigma); %!!!!Equation (2)!!!!!
% Can magnitude take the place of |ci - cj|£¿

tic
for l=1:3 % Why loop for 3 times?

    fprintf('Find Gaussian mixture models...\n');
    fprob = mixture_prob(I, K, 10, mask);
    bprob = mixture_prob(I, K, 10, 1-mask);
    prior = reshape(fprob ./ (fprob + bprob), size(I,1), size(I,2), 1); %set up energy
    %showgrey(prior)
    toc

    fprintf('Find minimum cut...\n');
    [u, erriter, i] = cmf_cut(prior, edge);
    mask = uint8(u>0.5); % Update the mask
    toc

end

segm = int16(u>0.5) + 1;

